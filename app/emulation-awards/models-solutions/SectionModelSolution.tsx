        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full md:w-auto">
          <div className="relative group flex-1 md:flex-initial">
            ...
            <select 
              ...
              className="w-full md:w-auto pl-12 pr-10 py-2.5 sm:py-3 bg-white rounded-xl text-xs sm:text-sm font-bold border border-slate-200 focus:border-blue-400 outline-none appearance-none cursor-pointer transition-all shadow-sm min-w-0 md:min-w-[180px]"
            >
              ...
              {filteredList.length > 0 ? (
                filteredList.map((item, index) => (
                  <tr key={item._id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="px-3 sm:px-4 py-3 sm:py-5 text-center font-bold text-slate-400 group-hover:text-blue-600">
                      {index + 1}
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-5 font-bold text-slate-700 leading-relaxed max-w-[240px] sm:max-w-none truncate sm:whitespace-normal">
                      {item.solution_model_name}
                    </td>
