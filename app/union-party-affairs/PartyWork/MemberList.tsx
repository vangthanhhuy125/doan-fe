  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex-1">
      <div className={`${colorClass} p-4 flex items-center gap-2 text-white`}>
        ...
        <button onClick={onAdd} className="ml-auto bg-white/20 p-1.5 rounded-lg hover:bg-white/30 transition-all border-none outline-none"><Plus size={16}/></button>
      </div>
        ...
        {data.length > 0 ? data.map((item: any) => (
          <div key={item._id} className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-transparent hover:border-slate-200 transition-all group">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => onEdit(item)} className="p-2 text-amber-500 hover:bg-amber-50 rounded-lg border-none bg-transparent outline-none"><Edit size={14}/></button>
              <button onClick={() => onDelete(item)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg border-none bg-transparent outline-none"><Trash2 size={14}/></button>
            </div>
          </div>
