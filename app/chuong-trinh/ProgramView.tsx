  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20">
        <div className="bg-[#0054a5] p-6 flex items-center justify-between text-white transition-colors">
        ...
        <div className="p-8 space-y-8 text-black">
          <div className="space-y-1">
            ...
            <h2 className="text-xl font-black text-[#0054a5] leading-tight">{data.program_name}</h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-6 bg-slate-50 p-4 rounded-2xl">
              ...
              <div className="space-y-1">
                ...
                <p className="font-bold text-gray-700 text-sm">{data.semester} - {data.academic_year}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="font-semibold text-gray-700">{data.stakeholders}</p>
          </div>

          <div className="grid grid-cols-2 gap-4 pt-6 border-t border-gray-100">
            <LinkBox label="Tài liệu" icon={<LinkIcon size={16}/>} href={data.source_url} colorClass="bg-blue-50 text-blue-600 border-blue-100" />
            <LinkBox label="Kế hoạch" icon={<FileText size={16}/>} href={data.plan_url} colorClass="bg-emerald-50 text-emerald-600 border-emerald-100" />
            <LinkBox label="Dự trù kinh phí" icon={<DollarSign size={16}/>} href={data.budget_url} colorClass="bg-amber-50 text-amber-600 border-amber-100" />
            <LinkBox label="Danh sách ĐRL" icon={<Award size={16}/>} href={data.training_score_list_url} colorClass="bg-purple-50 text-purple-600 border-purple-100" />
          </div>
        </div>
      </div>
    </div>
