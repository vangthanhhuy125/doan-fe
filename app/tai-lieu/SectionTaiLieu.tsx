      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-wrap items-center gap-4 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="pl-12 pr-10 py-3 bg-white rounded-xl text-sm font-bold border-none outline-none focus:ring-2 ring-blue-400 shadow-sm appearance-none min-w-[280px] cursor-pointer transition-all">
          ...
          {(searchTerm || filterCat) && (
            <button onClick={() => {setSearchTerm(""); setFilterCat("");}} className="p-3 bg-white text-red-500 rounded-xl shadow-sm hover:bg-red-50 transition-all active:scale-90">
              <RotateCcw size={18} />
            </button>
          )}
        </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredList.length > 0 ? filteredList.map((item: any, index: number) => (
              <tr key={item._id} className="hover:bg-blue-50/30 transition-colors group">
                ...
                <td className="px-6 py-4 font-bold text-slate-700 leading-relaxed">{item.document_name}</td>
                <td className="px-6 py-4 text-center font-bold text-slate-500">{item.semester}</td>
                <td className="px-6 py-4 text-center font-bold text-slate-500">{item.academic_year}</td>
                <td className="px-6 py-4 font-medium text-slate-500 italic">{item.document_type}</td>
                <td className="px-6 py-4 text-center">
                  <a href={item.document_url} target="_blank" className="inline-flex items-center gap-1 text-blue-600 hover:underline font-bold text-xs bg-blue-50 px-3 py-1 rounded-full no-underline"><LinkIcon size={12} /> Xem file</a>
                </td>
