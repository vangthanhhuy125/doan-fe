...
export default function SectionCTTN({ cttnList, onOpenModal }: Props) {
  ...
  const years = Array.from(new Set(cttnList.map(item => item.year))).filter(Boolean).sort().reverse();
    ...
    <section className="space-y-6 text-black">
      <div className="flex items-center justify-between border-b-2 border-[#0054a5] pb-3">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#0054a5] text-white px-4 py-2 rounded-lg font-bold shadow-lg hover:bg-[#1d92ff] transition-all active:scale-95 text-xs uppercase tracking-wider"
        >
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-wrap items-center gap-4 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          {(searchTerm || filterYear) && (
            <button onClick={() => {setSearchTerm(""); setFilterYear("");}} className="p-3 bg-white text-red-500 rounded-xl border border-red-100 hover:bg-red-50 shadow-sm"><RotateCcw size={18} /></button>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden">
        <table className="w-full text-sm text-left">
          <thead className="bg-[#0054a5] text-white font-bold text-[13x] tracking-widest text-center">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredList.length > 0 ? filteredList.map((item, index) => (
              <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                ...
                <td className="px-6 py-4 text-center font-bold text-slate-400 group-hover:text-[#0054a5]">{item.year}</td>
                <td className="px-6 py-4 text-center">
                  <div className="flex items-center justify-center gap-2">
                    <button onClick={() => onOpenModal('view', item)} className="p-2 text-[#0054a5] hover:bg-blue-100 rounded-lg transition-all shadow-sm border border-transparent hover:border-blue-200"><Eye size={18} /></button>
