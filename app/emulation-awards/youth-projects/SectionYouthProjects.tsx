    ...
    <section className="space-y-6 text-black">
      <div className="flex items-center justify-between border-b-2 border-[#0054a5] pb-3">
        ...
        {canCreate && (
          <button 
            ...
            className="flex items-center gap-2 bg-[#0054a5] text-white px-4 py-2 rounded-lg font-bold shadow-lg hover:bg-[#1d92ff] transition-all active:scale-95 text-xs uppercase tracking-wider border-none outline-none cursor-pointer"
          >
            <Plus size={16} /> <span>Thêm CTTN</span>
          </button>
        )}
      </div>
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-wrap items-center gap-4 shadow-sm">
        <div className="relative flex-1 min-w-[250px] group">
          ...
          <input 
            type="text" 
            placeholder="Tìm tên công trình hoặc đơn vị..." 
            value={searchTerm} 
            onChange={(e) => setSearchTerm(e.target.value)} 
            className="w-full pl-12 pr-4 py-3 bg-white rounded-xl text-sm border border-slate-200 focus:border-blue-400 outline-none shadow-sm transition-all font-medium" 
          />
        </div>
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <select 
              ...
              className="pl-12 pr-10 py-3 bg-white rounded-xl text-sm font-bold border border-slate-200 focus:border-blue-400 outline-none appearance-none cursor-pointer shadow-sm min-w-[180px]"
            >
          ...
          {(searchTerm || filterYear) && (
            <button 
              ...
              className="p-3 bg-white text-red-500 rounded-xl border border-red-100 hover:bg-red-50 shadow-sm border-none outline-none cursor-pointer"
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredList.length > 0 ? (
                  ...
                  {hasAnyAction && (
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {canView && (
                          <button onClick={() => onOpenModal('view', item)} className="p-2 text-[#0054a5] hover:bg-blue-100 rounded-lg transition-all shadow-sm border border-transparent hover:border-blue-200 bg-transparent outline-none cursor-pointer" title="Xem chi tiết">
                            <Eye size={18} />
                          </button>
                        )}
                        {canEdit && (
              ...
              <tr>
                <td colSpan={hasAnyAction ? 5 : 4} className="px-6 py-16 text-center italic text-slate-400 font-bold">
